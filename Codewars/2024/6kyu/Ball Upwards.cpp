...
{
public:
    static int maxBall(int v0_km_per_hr) {
        double v0 = v0_km_per_hr / 3.6; // Convert velocity to m/s
        const double g = 9.81; // Acceleration due to gravity in m/s^2
        double max_height = 0.0;
        int time_of_max_height = 0;

        for (int t = 0; ; ++t) { // Iterate over time in tenths of seconds
            double height = v0 * (t / 10.0) - 0.5 * g * (t / 10.0) * (t / 10.0);
            if (height > max_height) {
                max_height = height;
                time_of_max_height = t;
            } else if (t > 0) {
                // If the height starts decreasing, break the loop
                break;
            }
        }

        return time_of_max_height;
    }
};
