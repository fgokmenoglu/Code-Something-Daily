 ...
 * Challenge: You are given a vector of numbers. The numbers each repeat a certain number of times. 
...
std::vector<int> oddOnesOut(const std::vector<int>& numbers) {
  ...
  // Removing elements with odd counts
  out.erase(std::remove_if(out.begin(), out.end(), 
                             [&counts](int element) { return counts[element] % 2 != 0; }),
              out.end());
