/*
 * You have an amount of money a0 > 0 and you deposit it with an interest rate of 
 * p percent divided by 360 per day on the 1st of January 2016. 
...
public:
  static std::string dateNbDays(double a0, double a, double p) {
    ...
    startDate.tm_year = 116; // Year since 1900
    startDate.tm_mday = 1;   // 1st day of January
    ...
    dateStream << std::setfill('0') << (startDate.tm_year + 1900) << "-"
               << std::setw(2) << (startDate.tm_mon + 1) << "-"
               << std::setw(2) << startDate.tm_mday;
