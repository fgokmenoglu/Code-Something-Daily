...
std::string plant(char seed, int water, int fert, int temp) {
    ...
    // Check if the temperature is in the normal growing range
    bool normalTemperature = (temp >= 20 && temp <= 30);

    if (normalTemperature) {
        // Construct the plant string for normal temperature
        std::string segment = std::string(water, '-') + std::string(fert, seed);
        for (int i = 0; i < water; ++i) {
            result += segment;
        }
    } else {
        // Construct the plant string for abnormal temperature (flowers die)
        std::string stem = std::string(water * water, '-');
        result = stem + seed;
    }
